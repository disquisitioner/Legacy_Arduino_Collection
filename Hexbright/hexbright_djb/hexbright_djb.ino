/* 
 * Custom Hexbright firmware based on the standard factory program.  The primary change
 * is modification to the button sequence such that single click still steps through
 * OFF -> LOW -> MEDIUM -> HIGH -> OFF but long button press in each on state (LOW,
 * MEDIUM, and HIGH) turns the unit off.  Long button press in OFF state still shifts
 * to BLINKING mode.
 * 
...
void loop()
{
  ...
  case MODE_HIGH:
    if (btnDown && !newBtnDown && (time-btnTime)>50)
      newMode = MODE_OFF;
    /* Long-press to turn off is redundant, but makes
     * for consistency with LOW & MEDIUM
     */
    if (btnDown && newBtnDown && (time-btnTime)>500)
      newMode = MODE_OFF_PREVIEW;
    break;
